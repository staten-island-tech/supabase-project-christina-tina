  ...
  <div>
    ...
    <div v-else>
      <div v-if="!gameStarted">
        <button @click="startGame" class="px-4 py-2 bg-purple-600 text-white rounded">
          Start Saga Trivia
        </button>
      </div>

      <div v-else class="m-auto flex flex-wrap items-center justify-center">
...
async function getQuestionsAndAnswers() {
  const { data, error } = await supabase.from('questions').select('*').ilike('category', '%saga%')
